...
def parseRange1 (start stop : Nat) :
    Parser (Parser.Error.Simple (OfList Nat) Nat) (OfList Nat) Nat Unit :=
...
def test1 (n : Nat) : UInt8 :=
  match Parser.run (parseRange1 0 n) (mkOfList <| List.range n) with
  | .ok _ _ => 0
  | .error _ _ => 1

def test2 (n : Nat) : UInt8 :=
  match Parser.run (parseRange2 0 n) (mkOfList <| List.range n) with
  | .ok _ _ => 0
  | .error _ _ => 1
