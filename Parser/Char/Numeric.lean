...
variable {σ} {s : σ} {ε Pos m} [Parser.Stream s Char Pos] [Parser.Error ε s Char Pos] [Monad m]

@[inline]
private def decNum (n : Nat := 0) : ParserT ε s Char Pos m (Nat × Nat) :=
  foldl (fun (r : Nat × Nat) (d : Fin 10) => (10 * r.1 + d, r.2+1)) (n,0) ASCII.digit

@[inline]
private def binNum (n : Nat := 0) : ParserT ε s Char Pos m (Nat × Nat) :=
  foldl (fun (r : Nat × Nat) (d : Fin 2) => (r.1 <<< 1 + d, r.2+1)) (n,0) ASCII.binDigit

@[inline]
private def octNum (n : Nat := 0) : ParserT ε s Char Pos m (Nat × Nat) :=
  foldl (fun (r : Nat × Nat) (d : Fin 8) => (r.1 <<< 3 + d, r.2+1)) (n,0) ASCII.octDigit

@[inline]
private def hexNum (n : Nat := 0) : ParserT ε s Char Pos m (Nat × Nat) :=
  foldl (fun (r : Nat × Nat) (d : Fin 16) => (r.1 <<< 4 + d, r.2+1)) (n,0) ASCII.hexDigit

/-- Parse a `Nat` -/
def parseNat (decimalOnly := true) : ParserT ε s Char Pos m Nat := do
...
/-- Parse an `Int` -/
def parseInt (decimalOnly := true) : ParserT ε s Char Pos m Int := do
...
@[inline]
def parseScientific (α) [OfScientific α] : ParserT ε s Char Pos m α := do
...
/-- Parse a `Float` -/
def parseFloat : ParserT ε s Char Pos m Float := do
  match ← option? (char '+' <|> char '-') with
