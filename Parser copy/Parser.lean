...
instance (σ ε τ m) [Parser.Stream σ τ] [Parser.Error ε σ τ] [Monad m] :
  ...
  seq f x s pos := f s pos >>= fun
    | .ok pos f => x () s pos >>= fun
      | .ok pos x => return .ok pos (f x)
      | .error pos e => return .error pos e
    | .error pos e => return .error pos e
  seqLeft x y s pos := x s pos >>= fun
    | .ok pos x => y () s pos >>= fun
      | .ok pos _ => return .ok pos x
      | .error pos e => return .error pos e
    | .error pos e => return .error pos e
  seqRight x y s pos := x s pos >>= fun
    -- The first case seems to be equivalent with | .ok s pos _ => y () s pos
    | .ok pos _ => y () s pos >>= fun
...
def setStream (s : σ) : ParserT ε σ τ m PUnit :=
  fun _ p => return .ok s PUnit.unit
-/
